...
lemma ofMulAction_trace
    ...
    LinearMap.trace k (X →₀ k) (Representation.ofMulAction k G X g)
    = (Nat.card {x : X // g • x = x} : k) := by
  letI : Fintype X := Fintype.ofFinite X
  let b := Finsupp.basisSingleOne (R := k) (ι := X)
  ...
  · apply Finset.sum_congr rfl
    intro x _
    dsimp [b]
    simp only [LinearMap.toMatrix_apply, Finsupp.basisSingleOne_repr,
      Finsupp.coe_basisSingleOne, ofMulAction_single, LinearEquiv.refl_apply]
...
theorem mul_average_left (g : G) :
    ↑(Finsupp.single g 1) * average k G = average k G := by
...
theorem mul_average_right (g : G) :
    average k G * ↑(Finsupp.single g 1) = average k G :=  by
